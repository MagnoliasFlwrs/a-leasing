...
const ReviewCard = ({item}) => {
    ...
    let shortText = item.text.length > 243 ? item.text.slice(0 , 243) : item.text;
    let hiddenText  =  item.text.slice(0 , 243) ? item.text.slice(243 ,  (item.text.length)) : null;
    ...
    return (
        <div className='review-card'>
            ...
            <div className="text">
                <p> {shortText} {hiddenText ?
                    <span
                        onClick={() => setModalActivity(modalActivity ? false : true)}>
                        {modalActivity ? 'Скрыть' : 'Читать далее'}
                    </span> : ''}
                </p>
                <div className="hidden-text-modal" ref={hiddenTextRef}
